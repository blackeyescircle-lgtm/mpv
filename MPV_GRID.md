...
`mpv-grid.exe` is the compatibility executable name for mpv MultiView, an
mpv v0.41.0 (`41f6a645`) fork for playing up to 16 independent timelines in
one native window. It keeps one mpv core, one `vo_gpu_next` output, one D3D11
device/swapchain and one audio endpoint.
...
- wheel / `Shift+wheel`: seek the hovered tile by 5 seconds
